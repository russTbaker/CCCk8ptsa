...
function mtScore(entry_id) {
    ...
    var url = 'http://www.blogs.com/cgi-bin/mt/mt-cp.cgi';
    ...
    xh.send('__mode=vote&blog_id=9&f=scored,count&jsonp=mtScore_cb&id=' + entry_id);
    return false;
}
...
function mtUpdateScores() {
    ...
    xh.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    xh.send('__mode=score&blog_id=9&f=scored,count&jsonp=mtScore_cb&id=' + entry_ids);
    return false;
}
...
function favorite(evt, entry_id, score_span) {
    ...
    xh.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    xh.send('__mode=vote&blog_id=9&f=scored,sum&jsonp=favorite_cb&id=' + entry_id);
    var span = document.getElementById(score_span);
    if (span) span.innerHTML = '<img src="http://www.blogs.com/mt-static/images/indicator.white.gif" height="10" width="10" alt="favoriting..." />';
...
function script_follow(id) {
    ...
    var xh = mtGetXmlHttp();
    if (!xh) return false;
    xh.open('POST', 'http://www.blogs.com/cgi-bin/mt/mt-cp.cgi', true);
    xh.onreadystatechange = function() {
        if (xh.readyState == 4) {
            if (xh.status && ( xh.status != 200 )) {
                // error - ignore
    ...
    if (span) span.innerHTML = '<img src="http://www.blogs.com/mt-static/images/indicator.white.gif" height="10" width="10" alt="Following..." />';
}
...
function script_leave(id) {
    ...
    var span = document.getElementById('following-status');
    if (span) span.innerHTML = '<img src="http://www.blogs.com/mt-static/images/indicator.white.gif" height="10" width="10" alt="Leaving..." />';
}
...
function mtFetchUser(cb) {
    ...
    if (( cb == 'mtSetUser' ) && mtGetUser()) {
        ...
        script.src = 'http://www.blogs.com/cgi-bin/mt/mt-comments.cgi?__mode=session_js&blog_id=9&jsonp=' + cb + '&ts=' + ts;
...
function mtCommentOnSubmit(f) {
    if (!mtRequestSubmitted) {
        ...
        if (f.armor)
            f.armor.value = 'f92d478ebe2e224f7636f1567e3424d9e1e7551b';
...
function mtSignIn() {
    ...
    var url = 'http://www.blogs.com/cgi-bin/mt/mt-cp.cgi?__mode=login&blog_id=9';
    if (is_preview) {
        if (document['comments_form']) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
        } else {
            url += '&return_url=http%3A%2F%2Fwww.blogs.com%2F';
        }
    } else {
...
function mtSignOut(entry_id) {
    ...
    var doc_url = document.URL;
    doc_url = doc_url.replace(/#.+/, '');
    var url = 'http://www.blogs.com/cgi-bin/mt/mt-cp.cgi?__mode=logout&static=0';
    if (is_preview) {
        if (document['comments_form']) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
...
function mtShowGreeting() {
    ...
    if (u && u.is_authenticated) {
        if (u.is_banned) {
            ...
            var user_link;
            if (u.is_author) {
                user_link = '<a href="http://www.blogs.com/cgi-bin/mt/mt-cp.cgi?__mode=edit&amp;return_to=' + encodeURIComponent(document.URL) + '&amp;blog_id=9';
                user_link += '">' + u.name + '</a>';
            } else {
...
var body_id, li_string;

$(document).ready(function() {
    // Hiding and showing sub-navigation
    if ($("body").attr("id") || ($("body").attr("id") != "home")) {
        li_string = "ul#nav li#link-" + $("body").attr("id");
        body_id = $("body").attr("id");
        $(li_string).children("a").addClass("open");
        $(li_string).children("ul").css("display", "block");
        $(li_string).children("ul").css("z-index", "1");
    }
    if ($("body").attr("class")) {
        sub_li_string = "ul#nav li ul li#link-" + $("body").attr("class");
        body_class = $("body").attr("class");
        $(sub_li_string).children("a").addClass("active-cat");
    }

    var subnav_loc = $("#leaderboard-ads").css("margin-top");

    $("ul#nav li").mouseover(function() {
        if (("ul#nav li#" + $(this).attr("id")) != li_string) {
            $(this).children("ul").css("display", "block");
            if (li_string) {
                $(li_string).children("a").removeClass("open");
            }
        }
    });
    $("ul#nav li:not(.non-vertical-link)").mouseover(function() {
        if (subnav_loc == "7px") {
            $("#leaderboard-ads").css("margin", "32px 0 0 0");
        }
    });
    ...
    $("ul#nav li ul.sub-nav").mouseover(function() {
        if (("ul#nav li#" + $(this).parent().attr("id")) != li_string) {
            $(this).parent().children("a").addClass("open");
    ...
    $("ul.sub-nav").mouseout(function() {
        if (("ul#nav li#" + $(this).parent().attr("id")) != li_string) {
            $(this).parent().children("a").removeClass("open");
    ...
    $("ul#nav li.non-vertical-link").mouseover(function() {
        $(li_string).children("ul").css("display", "none");
    });
    $("ul#nav li.non-vertical-link").mouseout(function() {
        $(li_string).children("ul").css("display", "block");
    });


    // Hides 'What is Blogs.com' box
    $("a#what-hide").click(function () {
        $("#what-box").hide();
    });

    // Controls the Featured Top 10 Carousel
    var bgTop = 0;
    $(".top10-nav .next").click(function () {
        bgTop = bgTop - 10;
        bgPosition = "0px " + bgTop + "px";
        $(".top10-status").css({backgroundPosition: bgPosition});
    });
    $(".top10-nav .previous").click(function () {
        bgTop = bgTop + 10;
        bgPosition = "0px " + bgTop + "px";
        $(".top10-status").css({backgroundPosition: bgPosition});
    });
    $(".top10-items").jCarouselLite({
        btnNext: ".top10-nav .next",
        btnPrev: ".top10-nav .previous",
        visible: 2,
        scroll: 2,
        speed: 500
    });

    // The Top 10 Accordian Widget
    // $("#recent-top10 ul").children("li:first").children("ul").show();
    // $("#recent-top10 ul li a.show-list").hover(function(){
    //	if($(this).parent().children("ul").css("display") == "none") {
    //		$("#recent-top10 ul li ul:visible").slideUp("slow");
    ...
    $("form#search-form").submit(function() {
        var search_value = $("form#search-form input.text-input").val();
        search_value = search_value.replace(/ +/g, "+");
        var search_value = encodeURIComponent(search_value);
        search_value = search_value.replace(/(%2B)/g, "+");
        var search_url = "http://www.blogs.com/search/" + search_value;
        window.location = search_url;
        return false;
    });

    $(".rss-img, .share-img").hover(function() {
...
function highlightTab(link_id) {
    $("a[id^='link-']").parent().removeClass('selected');
    $("#link-" + link_id).parent().addClass('selected');
}
